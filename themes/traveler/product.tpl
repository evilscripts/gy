				...
				<div class="box-info-product">
					...
					<div class="product_attributes clearfix">
						...
						<p id="quantity_wanted_p">
							...
							<input name="prod_arrival_date" type="text" name="qty" id="prod_arrival_date" class="form_control text_datepicker" value="{$smarty.now|date_format:'%Y-%m-%d'}" style="width: 50%" />
		...
		{if $product->description}
			...
			<section class="page-product-box" style="padding: 30px">
				...
				{if !empty($product->address)}
					<b>Location: </b>{$product->address}<br/>
				{/if}

				{if !empty($product->address2)}
					<b>Location (Line 2): </b>{$product->address2}<br/>
				{/if}
