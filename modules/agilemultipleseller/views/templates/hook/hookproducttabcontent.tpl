...
<script type="text/javascript">
   ...
   {ldelim}
        ...
        initializeMap({$product->latitude},{$product->longitude} , 12, "map_canvas_1");
        loc = new google.maps.LatLng({$product->latitude}, {$product->longitude});
        addMarker('0',loc);
  ...
  <div class="row">
		<div class="margin-form clearfix" style="float:left;margin-left:20px;">
		    ...
		    <td valign="top" style="padding:10px;">
		        ...
		        {/if} *}
		        <b>{l s='Country:' mod='agilemultipleseller'}</b><br/>
		        {$sellerInfo->country}<br />
		        {if $sellerInfo->state}
		        <b>{l s='State:' mod='agilemultipleseller'}</b><br />{$sellerInfo->state}<br />
		        {/if}
                <br />
		...
		<div class="margin-form" style="float:right; display:none">
