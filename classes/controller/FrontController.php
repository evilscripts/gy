...
class FrontControllerCore extends Controller
{
	...
	public function initHeader()
	{
        ...
        $categories = $this->getCategories((int)self::$cookie->id_lang);
        $blankOption = array(
            0 => array(
            'id_category' => 0,
            'name' => ''
        ...
        $newFormatCat = $this->getNewCategoryFormat();

        $cate = array_merge($blankOption, $categories);
        self::$smarty->assign(array(
            'categories' => $cate,
            'new_categories' => $newFormatCat
        ));
	}

    protected function getCategories($id_lang)
    {
        $sql = 'SELECT id_category, id_shop, id_lang, name FROM '._DB_PREFIX_.'category_lang WHERE id_lang = '.$id_lang .' ORDER BY id_category';
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($sql);
        return $result;
    }

    private function getNewCategoryFormat() {
    	...
    	foreach($categories as $k => $v) {
            ...
            if($id_parent == 2 && $id_parent != 1) {
            	$newmain[$k]['id_category'] = $id_category;
            	$newmain[$k]['id_parent'] = $id_parent;
            	$newmain[$k]['name'] = $name;
            } else if($id_parent != 2 && $id_parent != 1){
            	$childCat[$k]['id_category'] = $id_category;
            	$childCat[$k]['id_parent'] = $id_parent;
            	$childCat[$k]['name'] = $name;
            }

    	}

    	$newmain = array_values($newmain);
    	$childCat = array_values($childCat);
    	$x = 0;
    	foreach ($childCat as $k => $v) {
            ...
            foreach($newmain as $key => $value) {
            	...
            	$id_category_main = $value['id_category'];
    			$id_parent_main = $value['id_parent'];
            	$name_main = $value['name'];

            	$newcat[$key]['id_category'] = $id_category_main;
            	$newcat[$key]['id_parent_main'] = $id_parent_main;
            	$newcat[$key]['name_main'] = $name_main;

            	if($id_parent == $id_category_main) {
            		$newcat[$key]['child'][$x]['id_category'] = $id_category;
            		$newcat[$key]['child'][$x]['id_parent'] = $id_parent;
            		$newcat[$key]['child'][$x]['name'] = $name;
            	}
    	...
    	foreach($newcat as $k => $val)
    		$newcat[$k]['child'] = array_values($val['child']);
